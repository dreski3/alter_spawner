...
export const runAlterGraph = async (
  ...
  while (pending.size) {
    ...
    // Queue the entire ready frontier. Executor lanes are acquired before a global
    // slot, so an OpenCode node waiting for SQLite ownership cannot crowd a direct
    // HTTP node out of the graph's usable concurrency.
    await Promise.all(
      ready.map(async (id) => {
        pending.delete(id);
