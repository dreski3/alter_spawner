...
import { createRuntime, createSpawnOptions, getHarness, HARNESS_ADAPTERS, registerHarness, spawnAlter, summarizeRunTree } from "../packages/core/src/index.js";
...
export const runNestedWorkload = async (item, { keepRun = false, outputRoot = null, model = null, environment = process.env,
  modelRun = (home, prompt, options) => getHarness("opencode").run(home, prompt, options) } = {}) => {
  const modelBacked = model != null;
  const runModel = model || "benchmark/mock";
  if (outputRoot && !keepRun) throw new Error("outputRoot requires keepRun");
  const root = outputRoot ? path.join(outputRoot, item.id) : mkdtempSync(path.join(tmpdir(), "mind-nested-benchmark-"));
  if (outputRoot) {
    if (existsSync(root)) throw new Error(`nested output already exists: ${root}`);
  ...
  try {
    ...
    writeFileSync(path.join(root, ".alters", "config.json"), JSON.stringify({
      ...
      retry: { same_harness_retries: modelBacked ? 0 : 1, fallback_retries: 0 },
      run_timeout_ms: modelBacked ? 900000 : 120000,
      providers: modelBacked ? {} : { benchmark: { models: { mock: { cost: { input_per_million: 0, output_per_million: 0 } } } } },
    }));
    registerHarness(executor, {
      async run(home, prompt, options) {
        ...
        if (modelBacked) {
          modelResult = await modelRun(home,
            `Reply with exactly DONE. This is benchmark node ${node.branch} at depth ${node.depth}.`, options);
        }
        if (!modelBacked && matches(item.retry, node) && options.attempt === 1) {
        ...
        if (!modelBacked && matches(item.failure, node)) {
        ...
        if (node.branch === "spine" && node.depth < item.max_depth) {
          ...
          await Promise.all(children.map((child) => spawnAlter(home, createSpawnOptions({
            ...
            textOnly: modelBacked,
            maxTokens: modelBacked ? 3000 : null,
        ...
        return modelResult || { ok: true, text: "done", tokens: tokens(), steps: 1, exitCode: 0, killed: false, budget_exceeded: false, empty_output: false, sessionID: null };
      },
    });
    const run = await spawnAlter(root, createSpawnOptions({
      ...
      textOnly: modelBacked,
      maxTokens: modelBacked ? 3000 : null,
    }), { runtime: modelBacked ? createRuntime({ env: environment }) : undefined });
    ...
    const passed = treeUsage.runs === expectedNodes(item) && treeUsage.max_depth === item.max_depth &&
      treeUsage.attempts === expectedNodes(item) + Number(!modelBacked && !!item.retry) &&
      (modelBacked || treeUsage.tokens.total === 3 * treeUsage.attempts) &&
      totalRetries === Number(!modelBacked && !!item.retry) && totalFailures === Number(!modelBacked && !!item.failure) &&
    ...
    return {
      ...
      per_depth: perDepth,
      ...(modelBacked ? { model: runModel, exact_answer_nodes: results.filter((result) => result.text?.trim() === "DONE").length } : {}),
    };
  } finally {
...
export const runNestedMatrix = async ({ workloads = loadTaskSet().set.nested_workloads, keepRuns = false, outputRoot = null } = {}) => {
  const cases = [];
  for (const item of workloads) cases.push(await runNestedWorkload(item, { keepRun: keepRuns, outputRoot }));
...
if (import.meta.url === pathToFileURL(process.argv[1] || "").href) {
  ...
  const outputRoot = outputIndex < 0 ? null : path.resolve(process.argv[outputIndex + 1]);
  const report = await runNestedMatrix({ keepRuns: process.argv.includes("--keep-runs"), outputRoot });
